...
function serachForKeyword($keyword) {
    ...
    // $stmt = $db->prepare("SELECT CONCAT(nom, ' ', prenom) as name FROM `employee` WHERE Nom LIKE ? OR Prenom LIKE ? ORDER BY name");
    $stmt = $db->prepare("SELECT CONCAT(nom, ' ', prenom) as name FROM `employee` where num_emp in (select distinct(num_encadrant) from stagiaire) AND (Nom LIKE ? OR Prenom LIKE ?)") ;
